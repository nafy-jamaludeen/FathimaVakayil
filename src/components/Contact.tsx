...
export default function Contact() {
  ...
  const [currentInquiryLink, setCurrentInquiryLink] = useState("");
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    // Construct the elegant formatted text markup
    const inquiryText = `Hello Fathima, I have created a new inquiry from your portfolio:\n\n*Name:* ${name}\n*Email:* ${email}\n*Inquiry Message:*\n"${message}"`;
    const targetWaUrl = `https://wa.me/97433819231?text=${encodeURIComponent(inquiryText)}`;
    
    setCurrentInquiryLink(targetWaUrl);
    ...
    setTimeout(() => {
      ...
      // Attempt to immediately open WhatsApp with details
      try {
        window.open(targetWaUrl, "_blank", "noopener,noreferrer");
      } catch (err) {
        // Fallback for strict sandbox iframe environments
        console.warn("Popup block caught; fallback to user manual click provided.");
      }
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-20">
          <div className="lg:col-span-5">
            ...
            <h2 className="font-display text-[2.5rem] md:text-[4rem] leading-none text-brand-offwhite tracking-tight">
              Let's craft the <span className="font-display italic text-brand-mint font-normal">future</span> together.
            </h2>
          </div>
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            ...
            <div className="p-8 rounded-2xl bg-white/5 border border-white/10 shadow-lg flex flex-col gap-4 text-left hover:border-brand-mint/30 transition-all duration-300 hover:scale-[1.03]">
              ...
              <div className="flex flex-col">
                ...
                <a 
                  href="mailto:fathimahudaifvakkail@gmail.com" 
                  className="font-serif text-[17px] sm:text-lg text-brand-offwhite font-bold hover:text-brand-mint transition-all cursor-pointer break-all"
                >
                  fathimahudaifvakkail@gmail.com
                </a>
                ...
                {isSent && (
                  ...
                  >
                    <div className="flex items-center gap-2 text-brand-mint font-bold text-sm">
                      ...
                      <span>Inquiry Configured! Ready for WhatsApp</span>
                    </div>
                    <p className="leading-relaxed text-brand-offwhite/95">
                      Thank you! We have formatted your coordinates and message blueprint. If your browser didn't automatically open WhatsApp, click the dispatch link below to send the details instantly:
                    </p>
                    {currentInquiryLink && (
                      <a
                        href={currentInquiryLink}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center justify-center gap-2 px-4 py-2.5 mt-2 bg-brand-mint text-brand-dark rounded-xl font-bold uppercase tracking-wider text-[10px] hover:scale-[1.02] active:scale-[0.98] transition-transform shadow-md cursor-pointer text-center"
                      >
                        <MessageCircle className="w-4 h-4 fill-brand-dark shrink-0" />
                        <span>Send inquiry to WhatsApp (+974 3381 9231)</span>
                      </a>
                    )}
                  </motion.div>
                )}
        ...
        <div className="mt-24 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex flex-col items-start gap-1">
            <span className="font-display text-lg font-bold text-brand-offwhite">Fathima Vakkayil</span>
          ...
          <div className="flex items-center gap-1 text-xs text-[#9FCBB3]/60 font-light font-sans">
            <span>© {new Date().getFullYear()} Fathima Vakkayil. Crafted with</span>
